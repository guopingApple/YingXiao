...
$(function() {
	...
	//
	$('.seviceCon .item:nth-child(3n)').css('border-right','0');
	//homepage height
	var wh = $(window).height();
	...
	$('.second').find('.box').hover(function(){
		...
		_this.stop().animate({'width':'52%','padding':'0 1px'},500);
		_this.find('.initialPic').stop().fadeOut(200);
		_this.find('.content').css('display','block');
	},function(){
		var _this = $(this);
		$('.second').find('.box').stop().animate({'width':'25%','padding':'0'},500);
		_this.find('.content').css('display','none');
